...
export default function ShareButton({
  ...
  const getEmailUrl = () => {
    ...
    const body = encodeURIComponent(
      `Check out this election information: ${url}`
    );
        ...
        {showDropdown && (
          <div className="absolute top-full right-0 mt-2 w-48 bg-slate-800 border border-white/10 rounded-lg shadow-xl z-50 overflow-hidden">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center px-4 py-3 text-sm text-slate-300 hover:text-orange-400 hover:bg-white/5 transition-colors"
              onClick={() => setShowDropdown(false)}
            >
              <svg
                className="w-4 h-4 mr-3"
